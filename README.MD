...
- The sample pipeline is built around `g++` (C++11 with `-Wall -Wextra -Wpedantic`) and demonstrates a full flow: read sources, run compilation, publish the binary, persist metadata/logs.
- Recommended run steps:
  1. Build the image: `docker build -t template_comp ./template `
